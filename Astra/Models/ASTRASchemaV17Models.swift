...
/// Frozen copies of the ten relationship-connected entities as they were
/// declared for schema V17.
///
/// V17 originally referenced the live `Workspace`/`AgentTask`/`TaskRun` graph
/// (the only change since V14 was `TaskRun.hasProtocolEvents`), so adding a
/// field to `AgentTask` for V18 would silently change the model fingerprint of
/// stores already written as 17.0.0. A frozen `TaskRun` cannot point at the
/// live `AgentTask` (whose `runs` inverse points back at the live `TaskRun`),
/// so the whole relationship closure has to be frozen together, exactly as
/// `ASTRASchemaV14Models` did at that version. Entities with no relationship
/// edge into this closure — `WorkspaceApp*`, `GoogleOAuthAccountProfile`,
/// `FeedbackReport`, `PersistentStoreMigrationRecord`, `TaskTurnRequest` —
/// stay live and keep their own declared shapes. Never edit these in place.
public enum ASTRASchemaV17Models {
  @Model
  ...
  public final class TaskRun {
    ...
    public var hasProtocolEvents: Bool?
    public var costUSD: Double = 0
