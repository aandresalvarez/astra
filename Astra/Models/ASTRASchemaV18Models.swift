...
/// declared by the production schema V18.
///
/// V18 originally referenced the live model graph and added only
/// `AgentTask.reasoningEffort`. Adding `TaskSchedule.reasoningEffort` later
/// changed that graph's fingerprint without changing the schema version, so
/// existing production V18 stores could no longer open. This snapshot restores
/// the exact V18 relationship closure; V19 owns the scheduled-task addition.
/// Never edit these models in place.
public enum ASTRASchemaV18Models {
  @Model
