...
/// declared for schema V14.
///
/// V14, V15 and V16 originally referenced the live `Workspace`/`AgentTask`/
/// `TaskRun` graph, so adding any field to those live models would silently
/// change the model fingerprint of stores already written as 14.0.0, 15.0.0 or
/// 16.0.0. A frozen `TaskRun` cannot point at the live `AgentTask` (whose
/// `runs` inverse points back at the live `TaskRun`), so the whole relationship
/// closure has to be frozen together. Entities with no relationship edge into
/// this closure — `WorkspaceApp*`, `GoogleOAuthAccountProfile`,
/// `FeedbackReport`, `PersistentStoreMigrationRecord`, `TaskTurnRequest` — stay
/// live and keep their own declared shapes. Never edit these in place.
public enum ASTRASchemaV14Models {
  @Model
