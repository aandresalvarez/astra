    ...
    private func regenerateDraft() {
        ...
        Task {
            ...
            await MainActor.run {
                ...
                statusMessage = Self.statusLine(for: result)
                isGeneratingDraft = false
            }
        }
    }

    private static func statusLine(for result: WorkspaceAppStudioGenerationResult) -> String {
        ...
        case .deterministicFallback:
            if let failure = result.providerFailure {
                return "Model unavailable — using a template draft. (\(failure))"
            }
            return "Model couldn't produce a valid manifest — using a template draft."
        }
    }
