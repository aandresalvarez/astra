    ...
    private var appearanceSettingsTab: some View {
        Form {
            ...
            Section("Privacy & Logging") {
                ...
                Text("When enabled, browser-control tasks receive ASTRA_BROWSER_DEBUG_CAPTURE=1. Failed browser actions persist a per-task browser-flight JSONL entry with a compact tree, console/navigation/network summaries, and a screenshot thumbnail. Controlled Chromium uses probed CDP event streams; embedded WebKit uses page instrumentation.")
                    .font(Stanford.caption(12))
                    .foregroundStyle(.secondary)
