    ...
    static func section(for task: AgentTask, codeDir: String) -> PromptContextSection? {
        ...
        if environment.workspaceCommandsRunInsideContainer {
            return PromptContextSection(
                ...
                Routing contract: provider reasoning runs on host macOS, workspace shell commands run in Docker, and host control-plane actions such as GitHub PR metadata, Jira, Google Cloud, SSH, browser, and Keychain access must use ASTRA-exposed host capabilities when available. Do not ask a subagent to "run locally" or to use native host Bash to escape this routing; subagents must use the same Docker workspace MCP tools for project commands. If a host control-plane capability is missing, report that capability as missing instead of trying to run a host CLI from the Docker workspace.
