...
/// Raw-envelope privacy gate shared by package construction and durable
/// adoption. Keeping one owner prevents replay paths from weakening intake.
public enum FeedbackContactMemberPolicy {
    private static let forbiddenNormalizedMemberNames: Set<String> = [
        "contact", "contactaddress", "contactemail", "contactemailaddress",
        "contactinfo", "contactinformation", "contactname", "contactphone",
        "contactphonenumber", "email", "emailaddress", "fullname", "phone",
        "phonenumber", "replyto", "reporter", "reportercontact",
        "reportercontactaddress", "reporteremail", "reporteremailaddress",
        "reportername", "reporterphone", "reporterphonenumber", "telephone"
    ]

    public static func containsForbiddenMember(in data: Data) -> Bool {
        ...
        if let dictionary = value as? [String: Any] {
            for (key, child) in dictionary {
                if forbiddenNormalizedMemberNames.contains(normalize(key))
                    || containsForbiddenMember(in: child) { return true }
            }
    ...
    private static func normalize(_ memberName: String) -> String {
        ...
        return memberName.precomposedStringWithCanonicalMapping
            .lowercased(with: Locale(identifier: "en_US_POSIX"))
            .unicodeScalars.filter { !separators.contains($0) }
...
public enum FeedbackRawCanonicalJSONVerifier {
    ...
    public static func isCanonicalObject(_ data: Data) -> Bool {
    ...
    private static func appendCanonical(_ value: Any, to output: inout String) -> Bool {
        ...
        case let string as String:
            ...
            appendCanonicalString(string, to: &output); return true
        case let number as NSNumber:
            let type = String(cString: number.objCType)
            if type == "c" { output += number.boolValue ? "true" : "false"; return true }
            ...
            output += number.stringValue; return true
        ...
        case let dictionary as [String: Any]:
            ...
            output += "{"
            for (index, key) in dictionary.keys.sorted(by: utf16LessThan).enumerated() {
                if index > 0 { output += "," }
                appendCanonicalString(key, to: &output); output += ":"
                guard let member = dictionary[key], appendCanonical(member, to: &output) else { return false }
            }
            output += "}"; return true
        default:
            return false
    ...
    private static func isNormalized(_ value: String) -> Bool {
        value.unicodeScalars.elementsEqual(FeedbackContractNormalizationV1.text(value).unicodeScalars)
    }
