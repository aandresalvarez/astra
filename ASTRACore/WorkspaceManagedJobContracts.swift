...
public enum WorkspaceManagedJobContractError: LocalizedError, Equatable, Sendable {
    ...
    case invalidRequestFingerprint
    case invalidRequestPayload
    case invocationPayloadMismatch
    ...
    public var errorDescription: String? {
        ...
        case .invalidRequestFingerprint: "Invalid workspace managed-job request fingerprint."
        case .invalidRequestPayload: "Invalid workspace managed-job request payload."
        case .invocationPayloadMismatch: "Workspace managed-job invocation id was reused with a different request."
        ...
        case .missingStartReceipt: "A nonterminal or successful workspace managed job requires a trusted start receipt."
        case .receiptJobMismatch: "Workspace managed-job receipt does not match its job record."
        }
    }
}

public enum WorkspaceManagedJobRequestFingerprint {
    public static func make(
        ...
        struct Payload: Encodable {
            ...
            var label: String?
            var progressProbe: String?
        }
        if let timeoutSeconds, !timeoutSeconds.isFinite {
            throw WorkspaceManagedJobContractError.invalidRequestPayload
        }
...
/// Durable evidence binding one backend job to the exact tool invocation that
/// admitted it. The invocation ID is an opaque, type-tagged MCP request ID.
public struct WorkspaceManagedJobStartReceipt: Codable, Equatable, Sendable {
    ...
    public static func make(
        ...
        return try WorkspaceManagedJobStartReceipt(
            ...
            externalIdentity: externalIdentity(taskID: taskUUID, runID: runUUID, jobID: canonicalJobID)
        )
    }
    ...
    public func validate(jobID: String? = nil) throws {
        ...
        guard !trimmedInvocationID.isEmpty,
              trimmedInvocationID == invocationID,
        ...
        guard requestFingerprint.range(
            of: #"^sha256:[0-9a-f]{64}$"#,
            options: .regularExpression
        ) != nil else {
            throw WorkspaceManagedJobContractError.invalidRequestFingerprint
        }
...
/// executor commands, host paths, log contents, and backend-private messages.
...
/// Authoritative backend record. This shape stays inside the trusted job store;
/// only `WorkspaceManagedJobStructuredResult` crosses tool/event boundaries.
