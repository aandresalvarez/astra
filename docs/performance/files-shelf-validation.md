...
The Files shelf used to animate its docked width. Because the shelf and task
transcript share an `HStack`, that animation interpolated the transcript width
for every frame. Long conversations then repeatedly reflowed visible Markdown
and selectable text on the main actor. On the production reproduction task
with 270 conversation items, shelf chrome took 1.3–3.5 seconds even though the
78-node filesystem scan took only 28–150 ms. Closing the shelf had the same
0.9–2.8 second symptom, confirming that scanning was not the primary cause.

Docked shelves now commit their final geometry once without animation. A true
inspector overlay may still animate because it does not change transcript
width. Index and preview work remain cancellable and derived from the
...
6. revalidates the current search revision before publishing cached or fresh
   results; and
7. cancels index and preview work as soon as the Files shelf is dismissed.
...
event=screen_transition_to_view_ready destination=shelf_markdown
event=screen_transition_to_view_ready destination=task_chat
```
...
The generic transition event additionally records `state_commit_ms`,
`state_to_view_ready_ms`, `max_main_actor_probe_gap_ms`, and
`main_actor_hitch_count`. These fields distinguish state mutation from a
main-actor layout/render stall; they do not claim to measure a GPU-presented
frame.
...
| Open/close on a 250+ item conversation | no width animation; transition p95 stays below 1 s |
