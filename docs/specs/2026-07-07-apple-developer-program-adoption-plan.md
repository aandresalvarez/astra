...
**Date:** 2026-07-07
**Status:** Phases 0–3 validated live and landed (see below). Phase 4's CI *mechanics* — cert import, signing, real Apple notarization, and a real public GitHub Release publish — are all proven end-to-end on a GitHub-hosted runner ([ASTRA v0.1.25](https://github.com/aandresalvarez/astra/releases/tag/v0.1.25)). **But that release's own appcast points at the wrong repo** (`susom/astra` instead of where it was actually published, `aandresalvarez/astra`) since neither `release.yml` nor the scripts' defaults were made self-consistent — see Phase 4 for the live evidence. This needs a human decision (which repo is the real distribution target) before it's trustworthy for real updates or before `push: tags: v*` is added. Phase 5 platform capabilities remain deliberately parked per their own table.
...
## Phase 4 — CI release automation — mechanics proven live; feed-URL target repo still undecided
...
**⚠️ Gap found while checking this claim, not yet resolved: the resulting appcast is not self-consistent, so this specific release does not actually function as a working update for anyone.** `script/release_update.sh:9` and `script/build_and_run.sh:97` both default `ASTRA_DOWNLOAD_URL_PREFIX`/the prod `SUFeedURL` to `https://github.com/susom/astra/releases/latest/download/...`, and `release.yml` doesn't override either. `git remote -v` shows this repo (`origin`, where every command in this whole Phase 4 effort has run and where `gh release create` actually publishes) is `aandresalvarez/astra` — a different repo than `susom/astra` (a separate configured remote). Checked live: the appcast actually published at `aandresalvarez/astra`'s v0.1.25 release has `<enclosure url="https://github.com/susom/astra/releases/latest/download/ASTRA-0.1.25.zip" .../>` — pointing at a repo that never received this build. A real client with the default prod `SUFeedURL` would fetch its appcast from `susom/astra` (not what was just tested here) and, even if it did see this appcast some other way, the enclosure URL 404s. **This CI validation proved the mechanics work; it did not prove a working update channel, and following this doc to enable the `push: tags: v*` trigger as-is would repeat the same broken appcast on every tagged build.**

**Before relying on this for real distribution or adding the tag trigger, someone needs to decide which repo is the actual distribution target** (`aandresalvarez/astra` or `susom/astra`) and either point `release.yml` at it explicitly (e.g. `ASTRA_DOWNLOAD_URL_PREFIX`/`ASTRA_SPARKLE_FEED_URL` env vars, or deriving them from `${{ github.repository }}` so the workflow is self-consistent wherever it runs) or run the workflow from that repo directly. That's a deployment decision outside what I'll pick on your behalf — flagging it here rather than guessing.
...
3. **Distribution-repo decision — ❌ open, blocks the rest.** v0.1.25's own appcast enclosure points at `susom/astra`, not `aandresalvarez/astra` where it was actually published — see the gap called out above. Needs a decision on the real target repo, then `release.yml` set up to match it (explicit `ASTRA_DOWNLOAD_URL_PREFIX`/`ASTRA_SPARKLE_FEED_URL`, or derived from `${{ github.repository }}`) before any of the below can be trusted.
4. **`push: tags: v*` trigger**: blocked on (3) — adding it now would make every tagged build repeat the same broken appcast.
5. **dSYM retention — ✅ confirmed working.** `ASTRA-0.1.25.dSYM.zip` is attached to the v0.1.25 release alongside the app zip and appcast (mechanically fine; same repo caveat as (3) applies to whether this is the *right* release).
6. Keep the local manual path working — CI is an automation of the same script, not a fork of it. (Unaffected by any of the above.)
