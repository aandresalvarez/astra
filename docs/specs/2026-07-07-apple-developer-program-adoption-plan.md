...
**Date:** 2026-07-07
**Status:** Phases 0–4 validated live and landed (see below). Phase 4's CI release pipeline — cert import, signing, real Apple notarization, and a real public GitHub Release publish — has all been proven end-to-end on a GitHub-hosted runner ([ASTRA v0.1.25](https://github.com/aandresalvarez/astra/releases/tag/v0.1.25)). Only `push: tags: v*` automatic triggering remains open, deliberately deferred as a follow-up now that the path it would trigger is proven. Phase 5 platform capabilities remain deliberately parked per their own table.
...
## Phase 4 — CI release automation — ✅ DONE, full pipeline proven live (2026-07-08)
...
Remaining scope — everything below is now done except the tag trigger, which is a deliberate follow-up rather than a blocker:
...
2. **Real notarization + publish dry run — ✅ done (2026-07-08).** Run [28922796918](https://github.com/aandresalvarez/astra/actions/runs/28922796918) with fresh `version=0.1.25 build=25` (never-before-released, avoiding the `--clobber` risk in the publish step's fallback path) exercised `notarytool submit` and `gh release create` live, producing [ASTRA v0.1.25](https://github.com/aandresalvarez/astra/releases/tag/v0.1.25).
3. **`push: tags: v*` trigger**: not yet added. Safe to add now that (2) is validated — a deliberate scheduling choice, not a blocker.
4. **dSYM retention — ✅ confirmed.** `ASTRA-0.1.25.dSYM.zip` is attached to the v0.1.25 release alongside the app zip and appcast.
5. Keep the local manual path working — CI is an automation of the same script, not a fork of it. (Unaffected by any of the above.)
