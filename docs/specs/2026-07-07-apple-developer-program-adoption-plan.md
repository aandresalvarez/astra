...
2. **Real notarization + publish mechanics — ✅ done (2026-07-08).** Run [28922796918](https://github.com/aandresalvarez/astra/actions/runs/28922796918) with fresh `version=0.1.25 build=25` (never-before-released, avoiding the `--clobber` risk in the publish step's fallback path) exercised `notarytool submit` and `gh release create` live, producing [ASTRA v0.1.25](https://github.com/aandresalvarez/astra/releases/tag/v0.1.25). The signing/notarization/publish API calls all genuinely work.
