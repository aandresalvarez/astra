...
4. **Fresh release to prove the fix — ❌ not yet done.** v0.1.25's appcast is still the broken one (fixing the defaults doesn't retroactively fix an already-published release). Needs one more real `workflow_dispatch` run with a fresh version to confirm the appcast is now self-consistent.
