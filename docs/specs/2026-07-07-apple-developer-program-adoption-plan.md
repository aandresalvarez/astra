...
**Date:** 2026-07-07
**Status:** Phases 0–4 validated live and landed (see below). Phase 4's full CI release pipeline — cert import, signing, real Apple notarization, publish, and a self-consistent Sparkle update feed — is proven end-to-end against `aandresalvarez/astra`, the confirmed real distribution target ([ASTRA v0.1.26](https://github.com/aandresalvarez/astra/releases/tag/v0.1.26)). Only `push: tags: v*` automatic triggering remains open, deliberately held as a separate decision (it's a standing authorization for every future tag push to auto-publish, not a one-time fix). Phase 5 platform capabilities remain deliberately parked per their own table.
...
## Phase 4 — CI release automation — ✅ DONE end-to-end; only the tag trigger remains, held as a separate decision
