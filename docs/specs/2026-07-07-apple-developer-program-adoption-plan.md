...
**Date:** 2026-07-07
**Status:** Phases 0–3 validated live and landed (see below). Phase 4's CI mechanics — cert import, signing, real Apple notarization, and a real public GitHub Release publish — are all proven end-to-end on a GitHub-hosted runner ([ASTRA v0.1.25](https://github.com/aandresalvarez/astra/releases/tag/v0.1.25)). That release's own appcast pointed at the wrong repo (a hardcoded `susom/astra` default, not `aandresalvarez/astra` where it was actually published) — now resolved: `aandresalvarez/astra` is confirmed as the real distribution target and every hardcoded default has been updated to match (see Phase 4). Still open: a fresh release to prove the corrected URLs actually work, and the `push: tags: v*` trigger. Phase 5 platform capabilities remain deliberately parked per their own table.
...
4. **Publish flow:** tag `vX.Y.Z` → `release_update.sh` produces `dist/release/ASTRA-<version>.dmg`, `dist/release/ASTRA-<version>.zip`, and `appcast.xml` → upload all three to the GitHub release. The DMG is the human first-install artifact; the zip is the Sparkle payload. Verify the zip asset name matches the appcast `--download-url-prefix` (`https://github.com/aandresalvarez/astra/releases/latest/download/`, `release_update.sh:9`) exactly — `latest/download/` URLs are name-sensitive. **Not done — deliberately.** Actually publishing to the real GitHub release is a public-content action outside what I'll do without your explicit go-ahead each time; the mechanics are proven, the trigger is yours.
...
## Phase 4 — CI release automation — mechanics proven live; distribution repo resolved, fresh release + tag trigger open
...
4. **Fresh release to prove the fix — ❌ not yet done.** v0.1.25's appcast is still the broken one (fixing the defaults doesn't retroactively fix an already-published release), and v0.1.25/v0.1.26 shipped only raw zip artifacts for manual installs. Needs one more real `workflow_dispatch` run with a fresh version to confirm the appcast is self-consistent and that the GitHub release includes the manual-install DMG.
5. **`push: tags: v*` trigger**: no longer blocked on the repo decision, but deliberately held as a separate call — enabling it is a standing authorization for every future `git push --tags` to auto-notarize-and-publish, not something to bundle into a URL-default fix.
