...
**Date:** 2026-07-07
**Status:** Phases 0–4 fully DONE and landed (see below). Phase 4's full CI release pipeline — cert import, signing, real Apple notarization, publish, and a self-consistent Sparkle update feed — is proven end-to-end against `aandresalvarez/astra`, the confirmed real distribution target ([ASTRA v0.1.26](https://github.com/aandresalvarez/astra/releases/tag/v0.1.26)). `push: tags: vX.Y.Z` automatic triggering is now wired up: pushing a release tag signs, notarizes, and publishes for real, with a defense-in-depth guard that fails loudly (rather than silently guessing a version) if a matched tag doesn't parse exactly. Phase 5 platform capabilities remain deliberately parked per their own table.
...
## Phase 4 — CI release automation — ✅ DONE end-to-end, including the tag trigger
...
**✅ `push: tags: vX.Y.Z` trigger — added (2026-07-08).** `on.push.tags` matches `v*.*.*` (broad on purpose — this glob dialect doesn't reliably support regex quantifiers like `[0-9]+`); the real strictness lives in the existing "Determine release version and build" step, which now has a defense-in-depth branch: if a tag push reaches that step without matching `^v[0-9]+\.[0-9]+\.[0-9]+$` exactly, it fails loudly (`exit 2`) instead of falling through to "use the latest existing tag," which would have silently published a different version than whatever was actually pushed. `skip_notarization` only applies to manual `workflow_dispatch` runs — a tag push always does the real notarize-and-publish path, since a pushed release tag is never meant to be a dry run. Not yet exercised live (that requires pushing a real tag, the next actual release); the workflow_dispatch path this reuses is already proven end-to-end above.
...
5. **`push: tags: vX.Y.Z` trigger — ✅ added (2026-07-08).** Pushing a well-formed release tag now signs, notarizes, and publishes for real, no manual `workflow_dispatch` needed. Includes a defense-in-depth guard: a matched-but-malformed tag fails loudly instead of silently publishing under a different (latest-existing) version.
