...
- `src/feedback/assessment/input_serializer`: serializes the sanitized report,
  issue summary, and exact source/current-main revision handles as untrusted
  data. It never emits shell fragments or interpolated commands.
...
- `src/feedback/assessment/output_validator`: validates model output against
  ...
  `FeedbackAssessmentValidator` before persistence.
