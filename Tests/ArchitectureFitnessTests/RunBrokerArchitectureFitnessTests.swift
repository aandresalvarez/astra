...
struct RunBrokerArchitectureFitnessTests {
    ...
    func soleCompositionRoot() throws {
        ...
        // The daemon composes its canonical RunLedger directly, so the
        // process-lifetime exclusive-writer lock must be claimed here; a
        // configuration that defaults the flag off silently loses it in
        // merges (that regression shipped once).
        #expect(brokerMain.contains("exclusiveWriter: true"))
