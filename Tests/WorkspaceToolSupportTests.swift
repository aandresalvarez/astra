...
struct WorkspaceToolSupportTests {
    ...
    func dockerWorkspaceJobManagerStartsDetachedDurableJob() throws {
        ...
        #expect(logLines.contains("if [ -n \"$kill_bin\" ] && \"$kill_bin\" -0 -- -\"$command_pid\" 2>/dev/null; then"))
    ...
    func dockerWorkspaceJobCancelTerminatesCommandProcessGroup() throws {
        ...
        #expect(logLines.contains("tr '\\0' ' ' < \"/proc/$1/cmdline\""))
        #expect(logLines.contains("grep -F -- \"$command_script\""))
        #expect(logLines.contains("safe_pid \"$target_pid\" || return 0"))
        #expect(logLines.contains("[ -n \"$kill_bin\" ] && \"$kill_bin\" -0 -- -\"$target_pid\" 2>/dev/null"))
        ...
        #expect(logLines.contains("elif pid_matches_managed_command \"$target_pid\" && kill -0 \"$target_pid\" 2>/dev/null; then"))
