...
struct WorkspaceToolSupportTests {
    ...
    func dockerWorkspaceJobManagerStartsDetachedDurableJob() throws {
        ...
        #expect(logLines.contains("for candidate in /usr/bin/setsid /bin/setsid /usr/sbin/setsid /sbin/setsid; do"))
        #expect(!logLines.contains("/usr/local/bin/setsid"))
    ...
    func dockerWorkspaceJobCancelTerminatesCommandProcessGroup() throws {
        ...
        #expect(logLines.contains("elif kill -0 \"$target_pid\" 2>/dev/null; then"))
