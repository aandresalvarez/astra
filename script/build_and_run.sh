...
# Trim leading/trailing whitespace (including a trailing newline from
...
# lookup using the raw secret still failed.
SIGN_IDENTITY="$(printf '%s' "$SIGN_IDENTITY_RAW" | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')"
